...
class EarmarkPorkRecord(BaseModel):
    """A model for representing earmark records."""
    title: str = Field(description="Summarized title of earmark")
    explanation: str = Field(description="Explanation of earmark")
    concern: str = Field(description="Why earmark is concerning")
    severity: Literal["low", "medium", "high"] = Field(description=SEVERITY_GUIDELINES)
    why: str = Field(description="Explain WHY this is an earmark or pork barrel provision. do not call things earmarks or pork barrel spending if they are not actually earmarks")
...
class EarmarkPorkRecords(BaseModel):
    """Container for a list of earmark records."""
    records: List[EarmarkPorkRecord] = Field(description="List of earmark records")
