...
from agent.types import AlignmentRecords, BeneficiaryRecords, BillCost, ChangeRecords, EarmarkPorkRecords, TrojanHorseRecords
...
def investigative_agent_subworkflow(state: State):
    ...
    def check_pork_barrel(chunks: list[Document]) -> list:
        """Identify potential pork-barrel spending and earmarks."""
        findings = []
        llm_struc = llm.with_structured_output(EarmarkPorkRecords)
        for chunk in chunks:
            response = llm_struc.invoke([
                PORK_BARREL_PROMPT,
                {"role": "user", "content": f"{chunk.page_content}{feedback_context}\nImportant: Only include earmarks and spending that are explicitly mentioned or can be directly inferred from the text."}
            ])
    ...
    def identify_trojan_horses(chunks: list[Document], original_purpose: str) -> list:
        ...
        for chunk in chunks:
            response = llm_struc.invoke([
                TROJAN_HORSE_PROMPT,
                {"role": "user", "content": f"Original purpose: {original_purpose}\nBill text: {chunk.page_content}{feedback_context}\nImportant: Only flag provisions that clearly deviate from the bill's stated purpose with specific evidence."}
            ])
            findings.append(response)
        return findings
    ...
    def identify_sleeper_provisions(chunks: list[Document]) -> list:
        ...
        for chunk in chunks:
            response = llm_struc.invoke([
                SLEEPER_PROVISION_PROMPT,
                {"role": "user", "content": f"{chunk.page_content}{feedback_context}\nImportant: Only include provisions with clear evidence of subtle but significant impact."}
            ])
            findings.append(response)
        return findings
    ...
    def analyze_beneficiaries(chunks: list[Document]) -> dict:
        ...
        for chunk in chunks:
            response = llm_struc.invoke([
                BENEFICIARY_ANALYSIS_PROMPT,
                {"role": "user", "content": f"{chunk.page_content}{feedback_context}\nImportant: Only include beneficiaries and benefits that are explicitly mentioned in the text."}
            ])
            findings.append(response)
        return findings
    ...
    investigation_results = {
        "earmarks": check_pork_barrel(text_chunks),
...
def correction_investigative_agent(state: State):
    ...
    def validate_findings(findings, finding_type: str, text_chunks: list[Document], context: str = ""):
        ...
        validation_criteria = {
            "earmarks": "Focus on verifying specific spending amounts and clear earmark designations. Ignore interpretative analysis.",
...
Ignore minor issues, subjective interpretations, or reasonable extrapolations."""
    ...
    validations = [
        ("earmarks", validate_findings(earmarks, "earmarks", text_chunks)),
...
if __name__ == "__main__":
    ...
    print("Earmarks and Directed Spending:")
    print(result["earmarks"] if result["earmarks"] else "No earmarks found.")
